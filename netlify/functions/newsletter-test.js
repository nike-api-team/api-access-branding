// ── TEMPORARY: Test-send newsletter to brett.kirby@nike.com only ──
// HTTP-invokable (not scheduled). Delete after confirming.
...
exports.handler = async (event) => {
  ...
  try {
    // Fetch latest content from live site (will use the just-pushed newsletter-content.json)
    ...
    if (!sendResponse.ok) {
      ...
      throw new Error('Send failed: ' + sendResponse.status + ' ' + err);
    }

    const result = await sendResponse.json();
    return {
      statusCode: 200,
      body: JSON.stringify({ message: 'Test sent to brett.kirby@nike.com', id: result.id, featured: featured.title }),
    };
  } catch (err) {
